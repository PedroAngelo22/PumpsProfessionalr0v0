...
# Importar as funções do nosso novo arquivo de banco de dados
from database import setup_database, save_project, load_project, get_user_projects, delete_project
...
if st.session_state["authentication_status"]:
    ...
    with st.sidebar:
        ...
        selected_project = st.selectbox("Carregar Projeto", user_projects, index=None, key="load_project_selector")
        
        col1, col2 = st.columns(2)
        if col1.button("Carregar", use_container_width=True, disabled=not selected_project):
            data = load_project(username, selected_project)
            if data:
        ...
        if col2.button("Deletar", use_container_width=True, disabled=not selected_project):
            delete_project(username, selected_project)
            st.success(f"Projeto '{selected_project}' deletado.")
            st.rerun()

        st.text_input("Nome do Projeto", key="project_name_input", value=selected_project or "")
        if st.button("Salvar Projeto", use_container_width=True):
            project_name = st.session_state.project_name_input
            if project_name:
                project_data = {
